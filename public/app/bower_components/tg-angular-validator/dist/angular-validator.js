			...
			link: function(scope, element, attrs, fn) {
					...
					if (scopeElementModel.$dirty || scope[element.form.name].submitted) {
						...
						if (scopeElementModel.$error.required) {
							...
							if ("required-message" in element.attributes) {
								$(element).popover({
									placement: 'top',
									content: element.attributes['required-message'].value
								});
							...
							// Display the default require message
								...
								$(element).popover({
									placement: 'top',
									content: defaultRequiredMessage
								});
								$(element).popover('show');
							}
							...
							if ("invalid-message" in element.attributes) {
								...
								$(element).popover({
									placement: 'top',
									content: element.attributes['invalid-message'].value
								});
								$(element).popover('show');
							}
							// Display the default error message
							else {
								console.log(element.name + " have been created");
								$(element).popover({
									placement: 'top',
									content: defaultInvalidMessage
								});
								$(element).popover('show');
							}
						}
				...
				function isValidationMessagePresent(element) {
					...
					for (var i = 0; i < elementSiblings.length; i++) {
						if (angular.element(elementSiblings[i]).hasClass("popover")) {
